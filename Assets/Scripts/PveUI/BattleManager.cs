...
public class BattleManager {
    ...
    private List<BattleRoleData> QuerySelfRoleList() {
        ...
        //result.Add(new BattleRoleData(GetBattlePosIndex(true, 3), 2, true));
        result.Add(new BattleRoleData(GetBattlePosIndex(true, 4), 2, true));
        //result.Add(new BattleRoleData(GetBattlePosIndex(true, 8), 2, true));
        return result;
    }
    ...
    private void CalCurRoundAtkOderList() {
        ...
        for (int i = 0; i < CurRoundAtkOderList.Count; i++) {
            str += CurRoundAtkOderList[i].QueryPos() + "(" + CurRoundAtkOderList[i].Speed + "), ";
        }
        //Debug.LogError(str);
    }
    ...
    public BattleRoleData GetCurHurtRoleData(BattleRoleData atkRoleData) {
        bool isSelf = atkRoleData.IsSelf();
        List<BattleRoleData> rivalList = new List<BattleRoleData>();
        for (int i = 0; i < this.BattleRoleList.Count; i++) {
            if (this.BattleRoleList[i].IsSelf() == !isSelf && this.BattleRoleList[i].CurHp > 0) {
                rivalList.Add(this.BattleRoleList[i]);
            }
        }
        //与最近的敌人之间的距离
        Dictionary<int, List<int>> nearStepDict = SearchPath.GetStepDict(atkRoleData.QueryPos());
        ...
        for (int i = 0; i < step.Count; i++) {
            ...
            for (int j = 0; j < rivalList.Count; j++) {
                for (int m = 0; m < curStepPosList.Count; m++) {
                    if (rivalList[j].QueryPos() == curStepPosList[m]) {
                        shortestStep = step[i];
                        break;
                    }
                }
        ...
        if (shortestStep != 0) {
            ...
            for (int i = 0; i < rivalList.Count; i++) {
                for (int j = 0; j < nearStepDict[shortestStep].Count; j++) {
                    if (rivalList[i].QueryPos() == nearStepDict[shortestStep][j]) {
    ...
    public bool IsBattleOver() {
        ...
        for (int i = 0; i < this.BattleRoleList.Count; i++) {
            if (!this.BattleRoleList[i].IsSelf() && this.BattleRoleList[i].CurHp > 0) {
                enermyOver = false;
                break;
            }
        }
    ...
    public List<int> GetLimitePosListBySearchPath(int targetPos) {
        ...
        for (int i = 0; i < this.BattleRoleList.Count; i++) {
            if (this.BattleRoleList[i].CurHp > 0 && this.BattleRoleList[i].QueryPos() != targetPos) {
                limitePosList.Add(this.BattleRoleList[i].QueryPos());
            }
        }
    ...
    public bool CanAtk(BattleRoleObj atker, BattleRoleObj hurt) {
        ...
        Dictionary<int, List<int>> nearStepDict = SearchPath.GetStepDict(atker.GetData().QueryPos());
        foreach (int step in nearStepDict.Keys) {
            for (int i = 0; i < nearStepDict[step].Count; i++) {
                if (nearStepDict[step][i] == hurt.GetData().QueryPos()) {
